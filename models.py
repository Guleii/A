...
class User(UserMixin, db.Model):
    __tablename__ = 'user'
    u_id = db.Column(db.Integer, primary_key=True, autoincrement=True)  # "id"
    username = db.Column(db.String(20))  # "用户名"
    password = db.Column(db.String(128))  # "密码"
    real_name = db.Column(db.String(30))  # 姓名
...
class Drug(db.Model):
    __tablename__ = 'drug'
    d_id = db.Column(db.Integer, primary_key=True, autoincrement=True)  # "序列号"
    d_name = db.Column(db.String(20), unique=True)  # "名称"
    inventory = db.Column(db.Integer)  # "库存"
    formulation = db.Column(db.String(20))  # "剂型"
    # variety = db.Column(db.String(20)) # "品种"（暂时使用分类来代替）
    period = db.Column(db.Integer)  # "有效期：/月"
